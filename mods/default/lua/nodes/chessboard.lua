--[[
mods/default/lua/nodes/chess.lua
================================
Realchess Mod (modified for Minetest TNG)
See https;//github.com/minetest-mods/realchess for details about the original
mod, which is licensed under the WTFPL.

Copyright (C) 2015-2016 kilbith <jeanpatrick.guerrero@gmail.com>
Copyright (C) 2015 beyondlimits
...
function realchess.init(pos)
	...
	local formspec = [[ size[8,8.6;]
			...
			background[0,0;8,8;default_chess_bg.png]
	...
	inv:set_list("board", {
		"default:chess_rook_black_1",
		"default:chess_knight_black_1",
		"default:chess_bishop_black_1",
		"default:chess_queen_black",
		"default:chess_king_black",
		"default:chess_bishop_black_2",
		"default:chess_knight_black_2",
		"default:chess_rook_black_2",
		"default:chess_pawn_black_1",
		"default:chess_pawn_black_2",
		"default:chess_pawn_black_3",
		"default:chess_pawn_black_4",
		"default:chess_pawn_black_5",
		"default:chess_pawn_black_6",
		"default:chess_pawn_black_7",
		"default:chess_pawn_black_8",
		'','','','','','','','','','','','','','','','',
		'','','','','','','','','','','','','','','','',
		"default:chess_pawn_white_1",
		"default:chess_pawn_white_2",
		"default:chess_pawn_white_3",
		"default:chess_pawn_white_4",
		"default:chess_pawn_white_5",
		"default:chess_pawn_white_6",
		"default:chess_pawn_white_7",
		"default:chess_pawn_white_8",
		"default:chess_rook_white_1",
		"default:chess_knight_white_1",
		"default:chess_bishop_white_1",
		"default:chess_queen_white",
		"default:chess_king_white",
		"default:chess_bishop_white_2",
		"default:chess_knight_white_2",
		"default:chess_rook_white_2"
	})
...
function realchess.move(pos, from_list, from_index, to_list, to_index, _, player)
	...
	if meta:get_string("winner") ~= "" then
		core.chat_send_player(playerName, "This game is over.")
		return 0
	end
	...
	if pieceFrom:find("white") then
		if playerWhite ~= "" and playerWhite ~= playerName then
			core.chat_send_player(playerName, "Someone else plays white pieces!")
			return 0
		end
		if lastMove ~= "" and lastMove ~= "black" then
			core.chat_send_player(playerName, "It's not your turn, wait for your opponent to play.")
			return 0
		end
	...
	elseif pieceFrom:find("black") then
		if playerBlack ~= "" and playerBlack ~= playerName then
			core.chat_send_player(playerName, "Someone else plays black pieces!")
			return 0
		end
			...
			if from_y - 1 == to_y then
				if from_x == to_x then
					...
					elseif to_index >= 1 and to_index <= 8 then
						inv:set_stack(from_list, from_index, "default:chess_queen_white")
					end
			...
			if from_y + 1 == to_y then
				if from_x == to_x then
					...
					elseif to_index >= 57 and to_index <= 64 then
						inv:set_stack(from_list, from_index, "default:chess_queen_black")
					end
				elseif from_x - 1 == to_x or from_x + 1 == to_x then
		...
		if thisMove == "white" then
			if from_y == 7 and to_y == 7 then
				if to_x == 1 then
					...
					if castlingWhiteL == 1 and idx57 == "default:chess_rook_white_1" then
						...
						inv:set_stack(from_list, 59, "default:chess_rook_white_1")
						check = false
					end
				elseif to_x == 6 then
					...
					if castlingWhiteR == 1 and idx64 == "default:chess_rook_white_2" then
						for i = from_index + 1, 63 do
							if inv:get_stack(from_list, i):get_name() ~= "" then
								return 0
							end
						end
						inv:set_stack(from_list, 62, "default:chess_rook_white_2")
		...
		elseif thisMove == "black" then
			if from_y == 0 and to_y == 0 then
				if to_x == 1 then
					...
					if castlingBlackL == 1 and idx1 == "default:chess_rook_black_1" then
						...
						inv:set_stack(from_list, 3, "default:chess_rook_black_1")
						check = false
					end
				elseif to_x == 6 then
					...
					if castlingBlackR == 1 and idx8 == "default:chess_rook_black_2" then
						for i = from_index + 1, 7 do
							if inv:get_stack(from_list, i):get_name() ~= "" then
								return 0
							end
						end
						inv:set_stack(from_list, 6, "default:chess_rook_black_2")
	...
	meta:set_int("lastMoveTime", core.get_gametime())

	if lastMove == "black" then
		core.chat_send_player(playerWhite, "["..os.date("%H:%M:%S").."] "..
				playerName.." moved a "..pieceFrom:match(":(%a+)")..", it's now your turn.")
	elseif lastMove == "white" then
		core.chat_send_player(playerBlack, "["..os.date("%H:%M:%S").."] "..
				playerName.." moved a "..pieceFrom:match(":(%a+)")..", it's now your turn.")
	end

	if pieceTo:sub(11,14) == "king" then
		core.chat_send_player(playerBlack, playerName.." won the game.")
		core.chat_send_player(playerWhite, playerName.." won the game.")
...
local function timeout_format(timeout_limit)
	local time_remaining = timeout_limit - core.get_gametime()
...
function realchess.fields(pos, _, fields, sender)
	...
	if fields.new and (playerWhite == playerName or playerBlack == playerName) then
		realchess.init(pos)
	elseif fields.new and lastMoveTime ~= 0 and core.get_gametime() >= timeout_limit and
			(playerWhite ~= playerName or playerBlack ~= playerName) then
		realchess.init(pos)
	else
		core.chat_send_player(playerName, "[!] You can't reset the chessboard, a game has been started.\n"..
...
function realchess.dig(pos, player)
	...
	return (lastMoveTime == 0 and core.get_gametime() > timeout_limit) or
		core.chat_send_player(playerName, "[!] You can't dig the chessboard, a game has been started.\n"..
...
core.register_node("default:chessboard", {
	description = "Chess Board",
	drawtype = "nodebox",
	paramtype = "light",
	paramtype2 = "facedir",
	inventory_image = "default_chessboard_top.png",
	wield_image = "default_chessboard_top.png",
	tiles = {"default_chessboard_top.png", "default_chessboard_top.png", "default_chessboard_sides.png"},
...
core.register_craft({
	output = "default:chessboard",
	recipe = {
		{"default:dye_black", "default:dye_white", "default:dye_black"},
...
local function register_piece(name, count)
	...
	if not count then
		core.register_craftitem("default:chess_"..name.."_"..color, {
			description = color:gsub("^%l", string.upper).." "..name:gsub("^%l", string.upper),
			inventory_image = "default_chess_"..name.."_"..color..".png",
			stack_max = 1,
			groups = {not_in_creative_inventory = 1}
		})
	else
		for i = 1, count do
			core.register_craftitem("default:chess_"..name.."_"..color.."_"..i, {
				description = color:gsub("^%l", string.upper).." "..name:gsub("^%l", string.upper),
				inventory_image = "default_chess_"..name.."_"..color..".png",
				stack_max = 1,
				groups = {not_in_creative_inventory = 1}
